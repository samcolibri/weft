//! WhatsAppDeleteMessage Node - Delete a message in a WhatsApp chat.
...
pub struct WhatsAppDeleteMessageNode;

#[async_trait]
impl Node for WhatsAppDeleteMessageNode {
    ...
    fn metadata(&self) -> NodeMetadata {
        NodeMetadata {
            ...
            inputs: vec![
                ...
                PortDef::new("fromMe", "Boolean", false),
            ],
            outputs: vec![
    ...
    async fn execute(&self, ctx: ExecutionContext) -> NodeResult {
        ...
        let from_me = ctx.input.get("fromMe").and_then(|v| v.as_bool()).unwrap_or(true);
        ...
        let action_req = ActionRequest {
            ...
            payload: serde_json::json!({ "chatId": chat_id, "messageId": message_id, "fromMe": from_me }),
        };
        ...
        match resp {
            ...
            Ok(_) => NodeResult::completed(serde_json::json!({ "success": false })),
            Err(e) => NodeResult::failed(&format!("Failed to reach bridge: {}", e)),
        }
    }
}

register_node!(WhatsAppDeleteMessageNode);
