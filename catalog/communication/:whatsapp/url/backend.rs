//! WhatsApp URL Node - Generate wa.me links
...
#[derive(Default)]
pub struct WhatsAppUrlNode;

#[async_trait]
impl Node for WhatsAppUrlNode {
    ...
    async fn execute(&self, ctx: ExecutionContext) -> NodeResult {
        ...
        let mut url = format!("https://wa.me/{}", clean_phone);
        
        if !message.is_empty() {
            url.push_str(&format!("?text={}", urlencoding::encode(message)));
        }
