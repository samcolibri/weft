//! SlackReact Node - Add a reaction to a Slack message.
//!
//! POST reactions.add
...
impl Node for SlackReactNode {
    ...
    async fn execute(&self, ctx: ExecutionContext) -> NodeResult {
        ...
        if emoji.is_empty() { return NodeResult::failed("Emoji name is required (e.g. 'thumbsup')"); }

        // Slack expects emoji name without colons
        let emoji_clean = emoji.trim_matches(':');
